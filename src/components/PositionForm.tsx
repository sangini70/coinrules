...
export function PositionForm() {
    ...
    <div className="rounded border bg-white p-4">
      ...
      {authUserInfo ? (
        ...
        <>
          ...
          <div className="text-sm text-gray-500">?黎??筌???????釉먮빱??/div>
          ...
          <div className="text-sm font-semibold">{authUserInfo.email ?? '?黎??筌??????????????쇨덧??}</div>
        ...
        <>
          ...
          <div className="text-sm text-gray-500">?棺??짆?????ш끽維??/div>
  ...
  */
  ...
  const liquidityBlockMessage =
    '??????????????????꾩룆梨???? ????釉먮폁?怨?땡?塋???????????嚥???癲??????????源녾텛??????????????????거???????????????ㅼ굣塋?\n??????熬곣뫖利당춯??쎾퐲?????????????????????????釉먮폁????????????????ㅻ깹????????遺얘턁????傭?끆?????⑤챷????';
...
const blockEntry = (message: string) => {
  ...
  const signalTrendUp = safeSignal?.trend === 'up';
    ...
    const conclusionMap = {} as Record<'AVOID' | 'RISK' | 'ENTRY' | 'OBSERVE' | 'WAIT', string>;
    const reasons = null;
